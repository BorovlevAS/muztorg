...
class PurchaseSubscriptionLine(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency",
        related="purchase_subscription_id.currency_id",
        store=True,
        readonly=True,
    )
    name = fields.Char(
    ...
    def _compute_subtotal(self):
        for record in self:
            ...
            taxes = record.tax_ids.compute_all(
                ...
                partner=record.purchase_subscription_id.partner_id,
            )
            record.update(
                {
    ...
    purchase_subscription_id = fields.Many2one(
        comodel_name="purchase.subscription", string="Subscription"
    )
    company_id = fields.Many2one(
        related="purchase_subscription_id.company_id",
        string="Company",
        store=True,
        index=True,
    )
    ...
    def _compute_name(self):
        for record in self:
            if not record.product_id:
                record.name = False
            lang = get_lang(
                self.env, record.purchase_subscription_id.partner_id.lang
            ).code
    ...
    @api.depends("product_id", "purchase_subscription_id.fiscal_position_id")
    def _compute_tax_ids(self):
        for line in self:
            fpos = (
                line.purchase_subscription_id.fiscal_position_id
                or line.purchase_subscription_id.fiscal_position_id.get_fiscal_position(
                    line.purchase_subscription_id.partner_id.id
                )
            )
    ...
    def _compute_price_unit(self):
        for record in self:
            if not record.product_id:
                continue
            if (
                record.purchase_subscription_id.pricelist_id
                ...
                product = record.product_id.with_context(
                    ...
                    pricelist=record.purchase_subscription_id.pricelist_id.id,
                    uom=record.product_id.uom_id.id,
                )
                record.price_unit = product._get_tax_included_unit_price(
                    record.company_id,
                    record.purchase_subscription_id.currency_id,
                    fields.datetime.now(),
                    "sale",
                    fiscal_position=record.purchase_subscription_id.fiscal_position_id,
