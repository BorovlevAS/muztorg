...
class PurchaseSubscription(models.Model):
    ...
    template_id = fields.Many2one(
        comodel_name="purchase.subscription.template",
        required=True,
    ...
    purchase_order_id = fields.Many2one(
        comodel_name="purchase.order", string="Origin sale order"
    )
    terms = fields.Text(
    ...
    tag_ids = fields.Many2many(comodel_name="purchase.subscription.tag", string="Tags")
    ...
    stage_id = fields.Many2one(
        comodel_name="purchase.subscription.stage",
        string="Stage",
        tracking=True,
    ...
    purchase_subscription_line_ids = fields.One2many(
        comodel_name="purchase.subscription.line",
        inverse_name="purchase_subscription_id",
    )
    purchase_order_ids_count = fields.Integer(
        compute="_compute_purchase_order_ids_count", string="Purchase orders"
    )
    account_invoice_ids_count = fields.Integer(
        compute="_compute_account_invoice_ids_count", string="Invoice Count"
    )
    close_reason_id = fields.Many2one(
        comodel_name="purchase.subscription.close.reason", string="Close Reason"
    )
        ...
        for subscription in self.search([]):
            if subscription.in_progress:
                if (
                    ...
                    and subscription.purchase_subscription_line_ids
                ):
                    try:
                        subscription.generate_invoice()
                    except Exception:
                        logger.exception("Error on subscription bill generate")
    ...
    @api.depends("purchase_subscription_line_ids")
    def _compute_total(self):
        for record in self:
            recurring_total = amount_tax = 0.0
            for order_line in record.purchase_subscription_line_ids:
    ...
    def action_start_subscription(self):
        ...
        in_progress_stage = self.env["purchase.subscription.stage"].search(
    ...
    def _prepare_purchase_order(self, line_ids=False):
        self.ensure_one()
        return {
    ...
    def create_invoice(self):
        ...
        line_ids = []
        for line in self.purchase_subscription_line_ids:
        ...
        invoice_id = (
            ...
            .sudo()
            .with_context(default_move_type="in_invoice", journal_type="purchase")
    ...
    def create_purchase_order(self):
        if not self.env["purchase.order"].check_access_rights("create", False):
            try:
                self.check_access_rights("write")
                self.check_access_rule("write")
            except AccessError:
        ...
        line_ids = []
        for line in self.purchase_subscription_line_ids:
            line_values = line._prepare_purchase_order_line()
            line_ids.append((0, 0, line_values))
        values = self._prepare_purchase_order(line_ids)
        order_id = self.env["purchase.order"].sudo().create(values)
        self.write({"purchase_order_ids": [(4, order_id.id)]})
        return order_id
        ...
        return {
            ...
            "domain": [
                (
                    "id",
                    "in",
                    self.invoice_ids.ids + self.purchase_order_ids.invoice_ids.ids,
                )
            ],
    ...
    def _compute_purchase_order_ids_count(self):
        ...
        for record in self:
            record.purchase_order_ids_count = count_dict.get(record.id, 0)

    def action_view_purchase_order_ids(self):
        active_ids = self.purchase_order_ids.ids
        return {
            "name": self.name,
            "view_type": "form",
            "view_mode": "tree,form",
    ...
    def create(self, values):
        if "recurring_rule_boundary" in values:
            if not values["recurring_rule_boundary"]:
                template_id = self.env["purchase.subscription.template"].browse(
                    values["template_id"]
                )
        ...
        if "date_start" in values and "recurring_next_date" in values:
            ...
            values["stage_id"] = (
                self.env["purchase.subscription.stage"]
