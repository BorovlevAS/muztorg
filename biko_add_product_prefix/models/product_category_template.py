    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        if name:
            ...
            if operator in positive_operators:
                category_ids = list(self._search([('biko_product_prefix_ids.id', 'in', _ids_link)] + args, limit=limit, access_rights_uid=name_get_uid))
            ...
            if not category_ids and operator not in expression.NEGATIVE_TERM_OPERATORS:
                ...
                category_ids = list(self._search(args + [('biko_product_prefix_ids.id', 'in', _ids_link)], limit=limit))
                ...
                domain = expression.OR([
                    ['&', ('biko_product_prefix_ids.id', 'in', _ids_link), ('name', operator, name)],
                    ['&', ('biko_product_prefix_ids', '=', False), ('name', operator, name)],
                ])
            ...
            if not category_ids and operator in positive_operators:
                ...
                if res:
                    _prefixes_ids = self.env['biko.product.prefix'].search([('name', operator, res.group(2))]).read(['id'])
                    _ids_link = [x['id'] for x in _prefixes_ids]
                    category_ids = list(self._search([('biko_product_prefix_ids.id', 'in', _ids_link)] + args, limit=limit, access_rights_uid=name_get_uid))
        else:
